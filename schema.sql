...
CREATE INDEX idx_exception_queue_reg ON exception_queue (extracted_reg_no);
...
CREATE INDEX idx_grades_status ON grades (prof_status);

-- Unique constraint: one grade per student per assessment
CREATE UNIQUE INDEX idx_grades_student_assessment ON grades (student_id, assessment_id);
...
CREATE INDEX idx_ledger_hashes_assessment ON ledger_hashes (assessment_id);
